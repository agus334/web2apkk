...
// ⏰ Mulai auto backup database setiap 1 jam
userService.startAutoBackup(bot);
...
bot.onText(/\/backup/, async (msg) => {
    ...
    try {
        await bot.sendMessage(chatId, '⏳ Mengirim backup database...', { parse_mode: 'HTML' });
        await userService.sendBackupToOwner(bot, chatId, 'manual');
        await bot.sendMessage(chatId, `
✅ <b>Backup Berhasil Dikirim!</b>
━━━━━━━━━━━━━━━━━━
👥 <b>Total Users:</b> <code>${userService.getCount()}</code>
📅 <b>Waktu:</b> ${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}
━━━━━━━━━━━━━━━━━━
<i>File users.json telah dikirim ke owner.</i>
        `.trim(), { parse_mode: 'HTML' });
    } catch (e) {
        await bot.sendMessage(chatId, `❌ <b>Gagal kirim backup:</b> ${e.message}`, { parse_mode: 'HTML' });
    }
});
